...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        self.input_placeholder = tf.placeholder(shape=[self.config.batch_size, self.config.n_features], dtype = tf.float32, name='input_placeholder')
        self.labels_placeholder = tf.placeholder(shape = [self.config.batch_size, self.config.n_classes], dtype = tf.float32, name = 'labels_placeholder')
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None):
        ...
        if labels_batch is None:
            feed_dict = {'input_placeholder:0': inputs_batch}
        else:
            feed_dict = {'input_placeholder:0': inputs_batch, 'labels_placeholder:0': labels_batch}
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('softmax') as scope:
            t = tf.sqrt(tf.cast(self.config.batch_size, tf.float32))
            W = tf.get_variable(name = 'W', initializer=tf.truncated_normal(shape=[100,5],mean=0.0, stddev=1/t, dtype=tf.float32), dtype = tf.float32)
            b = tf.get_variable(name = 'b', initializer=tf.zeros(shape = [1,5], dtype = np.float32),dtype=tf.float32)
        linear_layer = tf.matmul(self.input_placeholder, W) + b
        pred = softmax(linear_layer)
        return pred
