...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        windowed_data_mod = []
        startAdd = [start*window_size]
        endAdd = [end*window_size]
        sentenceWithTerminals = startAdd+ sentence+ endAdd
        windowed_x = [sentenceWithTerminals[t:t+1+2*window_size] for t in range(len(sentence))]
        new_entry = []
        for i in windowed_x:
            new_entry = []
            for j in i:
                new_entry.extend(j)
            windowed_data_mod.append((new_entry))
            windowed_data_modA = [i for i in zip(windowed_data_mod, labels)]
        windowed_data.extend(windowed_data_modA)
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.dropout_placeholder = tf.placeholder(dtype = tf.float32, shape = (), name = 'dropout_value')
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddingsPull, shape = [-1, self.config.n_window_features*self.config.embed_size] )
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable(name = 'U', dtype = tf.float32, shape = [self.config.hidden_size, self.config.n_classes],
                            initializer = tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable(name = 'b1', dtype = tf.float32, shape = [1,self.config.hidden_size],
                                initializer = tf.zeros_initializer())
        b2 = tf.get_variable(name = 'b2', dtype = tf.float32, shape = [1,self.config.n_classes],
                                initializer = tf.zeros_initializer())
