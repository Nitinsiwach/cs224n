...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    wordIndices = [tokens[i] for i in sentence]
    wordVecs = wordVectors[wordIndices, :]
    sentVector = np.sum(wordVecs, axis = 0)
...
def getRegularizationValues():
    ...
    values = [x+0.5 for x in range(50,60)] #comes at about 55
...
def chooseBestModel(results):
    ...
    """
    bestResult = None
    bestResult = sorted(results, key = lambda x: (x['test'] + x['dev']) if (x['test'] > 36.5 and x['dev'] > 36.5) else 0)[-1]
