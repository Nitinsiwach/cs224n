...
def pad_sequences(data, max_length):
    ...
    maskBin = [zero_vector]*max_length
    maskLabel = [4]*max_length
    #print("Zerovec", zero_vector, "zero label", zero_label, "maskbin", maskBin, "masskLabel", maskLabel)
    for sentence, labels in data:
        if len(sentence) > max_length:
            sentence = sentence[:max_length]
            labels = labels[:max_length]
        sentenceN = sentence + maskBin[len(sentence):]
        sentenceN = sentenceN[:max_length]
        labelsN = labels + maskLabel[len(labels):]
        labelsN = labelsN[:max_length]
        mask = ([True]*len(sentence) + [False]*(max_length - len(sentence)))
        ret.append((sentenceN, labelsN, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        self.input_placeholder = tf.placeholder(name = 'input_placeholder', 
                                                shape = (None, self.max_length, self.config.n_features), dtype = tf.int32)
        self.labels_placeholder = tf.placeholder(name = 'labels_placeholder',
                                                 shape = (None, self.max_length), dtype = tf.int32)
        self.mask_placeholder = tf.placeholder(name = 'mask_placeholder',
                                               shape = (None, self.max_length), dtype = tf.bool)
        
        self.dropout_placeholder = tf.placeholder(name = 'dropout_placeholder',
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, shape = (-1, self.config.max_length, self.config.n_features*self.config.embed_size))
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        xUse = tf.transpose(x, perm = [1,0,2])
        ho = tf.zeros((tf.shape(xUse)[1],self.config.hidden_size,))
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                ho,ot = cell(xUse[time_step,:,:], ho, "RNN")
                ot_dropout = tf.nn.dropout(ot, keep_prob = dropout_rate)
                pred = tf.matmul(ot_dropout, U) + b2
                preds.append(pred)
    ...
    def add_loss_op(self, preds):
        ...
        ### END YOUR CODE
        lossNoMask = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = self.labels_placeholder, logits = preds)
        loss = tf.reduce_mean(tf.boolean_mask(lossNoMask, self.mask_placeholder))
        return loss
